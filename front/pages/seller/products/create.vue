    ...
    <el-card class="shadow-lg">
      ...
      <el-form :model="form" :rules="rules" ref="formRef" label-position="top" label-width="100px" class="space-y-4">
        <div class="flex flex-row-reverse justify-between items-center gap-2">
          ...
          <el-form-item label="Produit" prop="product_name" class="w-75">
        ...
        <div v-if="res_products.length > 0" class="flex flex-col gap-4 py-2" style="max-height: 400px; overflow-y: auto; cursor: pointer;">
          ...
          shadow="hover"
          >
          <el-text tag="b" style="font-size: 16px; line-height: 24px;">{{ product.name }}</el-text> <!-- 增加文字尺寸 -->
          </el-card>
        </div>

  <!-- Product ID -->
        <el-form-item label="ID Produit" prop="productId">
          <el-input v-model="form.productId" placeholder="Entrez l'ID du produit" type="number" />
        </el-form-item>
...
const rules = {
  ...
  quantity: [
    ...
    { type: "number", message: "Doit être un numéro valide", trigger: "change" },
  ],
  brand: [
    { required: true, message: "Veuillez sélectionner une marque", trigger: "change" },
  ],
};
...
const loadBrands = async (query = "", page = 1) => {
  ...
  try {
    ...
    if (response.status === 200) {
      ...
      if (page === 1) {
        listBrands.value = brands; // 如果是第一页，覆盖品牌列表
      } else {
        listBrands.value = [...listBrands.value, ...brands]; // 否则追加内容
      }

      if (brands.length < 20) {
        hasMoreBrands.value = false; // 没有更多数据
      } else {
        currentPage.value++;
      }
    } else {
