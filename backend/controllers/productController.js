...
class ProductController {
    ...
    static async getPaginatedUniqueBrands(req, res) {
        try {
            // 获取分页参数
            const { page = 1, limit = 20 } = req.query; // 默认第一页，每页显示 20 条
            ...
            console.error('Error fetching paginated unique brands:', error);
            return res.status(500).json({ message: '无法获取品牌列表！' });
        }
    }
